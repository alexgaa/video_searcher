...
class FileWork {
    ...
    public function CsvReadFile($file_name,$separator='|'){
        ...
        foreach ($file as $row) {
            if($row[0]!=="|"){
                ...
                if($max_string_arr!=count($arra_t)){  // отлавливаем спец символ
                    unlink($file_name);
                    $this->error="Ошибка! В файле <b>'".substr(strrchr($file_name,'/'),1)."'</b> ! Строка ".$i." содержит спец символ | или перенос строки(ctrl+j), данные не загруженны, удалите/замените в файле все символы | и переносы строки в ячейке(ctrl+j) !";
                    return false;
                }
                $i++;
            }
            else {
                 $this->error='Ошибка! Входящий файл содержал пустую строку!</b>';
            }
        }
        ...
        if($this->Max_string<=1 and $this->Max_colum<=1){
            unlink($file_name);
            $this->error=" Ошибка! Файл ".substr(strrchr($file_name,'/'),1)." - пустой!!!";
            return false;
        }
        unlink($file_name);
        return $this->arr;
    ...
    */
    public function checkColumArray($nameColum,$array_users){
        ...
        foreach ($nameColum as $name){
            ...
            if($number===false){
                $this->error="Ошибка! Не найден столбец: <b>".$name."</b> - загрузка отменена!";
                return false;
            }
            else{
    ...
    public static function csvWriteFile($name_file,$arr,$separator='|'){
        $a="";
        $session = new Session();
        $tempdir =DIR_TEMP.$session->get('dir_user').'/';
