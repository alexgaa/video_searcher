...
class FinalAnalysis 
{
    ...
    public function allDataCheck($link, $arrayAllData)
    {
        ...
        if ($this->chekOldOrNewData($link)){
            ...
            //ранее запрашивалась
            $resultData=$this->ComparisonNewAandOldData($link, $arrayAllData);
            if(!$resultData){
                return false;
            } else{
                return $resultData;
    ...
    public function chekOldOrNewData($link)
    {
        ...
        $sql="SELECT input_url.id FROM `input_url` INNER JOIN `user_data` ON input_url.id_user=user_data.id WHERE user_data.name=:name AND input_url.url_key=:url_key";
     ...
     * 
     */
    public function ComparisonNewAandOldData($link,$inputAllDataArray)
    {   
    ...
    {
        $db=DB::getDB(DB_SEARCH_VIDEO);
        $sql="SELECT * FROM video_data WHERE id_input_url='".$this->id_input_url."'";
        $arrayDb=$db->get($sql);
        if(empty($arrayDb)){
            return false;
        } else {
